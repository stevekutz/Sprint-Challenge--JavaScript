...
let tyrannosaurus = new DinoMaker({
  info: {
    name: 'Tyrannosaurus',
    diet: 'carnivorous',
    weight: '7000kg',
...
let velociraptor = new DinoMaker({
  info: {
    ...
    length: '1.8m',
    period: 'Late Cretaceious',
    noise: 'SSSSSSsssssssssssss, MMMMmmmmm , that looks yummy!!! '
  }
...
console.log(tyrannosaurus.info.weight);

// What was the diet of a velociraptor?
console.log(velociraptor.info.diet);

// How long was a stegosaurus?
console.log(stegosaurus.info.length);

// What time period did tyrannosaurus live in?
console.log(tyrannosaurus.info.period);
...
console.log(velociraptor.roar()); //   SSSSSSsssssssssssss, MMMMmmmmm , that looks yummy!!!!
...
const uni = [];
function getUniNames(arr){
  for(let i = 0; i < arr.length; i++) {
    if(arr[i].university.includes('Uni')) uni.push(arr[i].university);
  }
  return uni;
}

getUniNames(graduates);
console.log(uni);
...
let lowerCase = [];
lowerCase = zooAnimals.map(el => lowerCase.push( el.animal_name.toLowerCase() ));
